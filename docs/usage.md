...
/workspaces/scflv_test_data/GEXSCOPE-V2
...
```
python scripts/samplesheet.py -m manifest.csv -f /workspaces/scflv_test_data/GEXSCOPE-V2
```
...
```
sample,fastq_1,fastq_2
Sample_X,/workspaces/scflv_test_data/GEXSCOPE-V2/test_R1.fastq.gz,/workspaces/scflv_test_data/GEXSCOPE-V2/test_R2.fastq.gz
Sample_Y,/workspaces/scflv_test_data/GEXSCOPE-V2/Sample_Y_S1_L001_R1_001.fastq.gz,/workspaces/scflv_test_data/GEXSCOPE-V2/Sample_Y_S1_L001_R2_001.fastq.gz
Sample_Y,/workspaces/scflv_test_data/GEXSCOPE-V2/Sample_Y_S1_L002_R1_001.fastq.gz,/workspaces/scflv_test_data/GEXSCOPE-V2/Sample_Y_S1_L002_R2_001.fastq.gz
```
...
```bash
nextflow run singleron-RD/scflv \
...
```bash
nextflow run singleron-RD/scflv -profile docker -params-file params.yaml
```
...
```
pip install nf-core
nf-core launch singleron-RD/scflv
```
...
```
nextflow run singleron-RD/scflv -profile test,docker --outdir results
```
...
```bash
nextflow pull singleron-RD/scflv
```
...
First, go to the [singleron-RD/scflv releases page](https://github.com/singleron-RD/scflv/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
